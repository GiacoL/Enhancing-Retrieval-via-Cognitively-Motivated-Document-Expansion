...
# Select the subset of corpus keys to generate texts 
start_subset = info_g_variables['g_start_subset']
end_subset = info_g_variables['g_end_subset']
...
###
# SELECT ONLY A SUBSET OF DOCUMENTS

## extract a list of all keys from the dictionary
corpus_keys = list(corpus_to_use.keys())

## select only the [start_subset:end_subset]
corpus_keys_to_use = corpus_keys[start_subset:end_subset+1]
# update the corpus_to_use with the subset of keys
corpus_to_use = {key: value for key, value in corpus.items() if key in corpus_keys_to_use}
...
# Save the output
import pickle
file_path = os.path.join(results_dir,'generated_text_dic_'+g_exp_gen+'.pkl')
...
if os.path.exists(file_path):
    ...
    for ed_i in existing_dic.keys():
        existing_dic[ed_i]['generated_documents'].update(generated_text_dic[ed_i]['generated_documents'])
    #existing_dic.update(generated_text_dic)
    
    # Save the updated dictionary back to the file
    with open(file_path, 'wb') as file:
        pickle.dump(existing_dic, file)
else:
    # Create a new file with the generated_text_dic
    with open(file_path, 'wb') as file:
